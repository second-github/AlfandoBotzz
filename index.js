    ...
    hisoka.ev.on('group-participants.update', async (anu) => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `𝙎𝙚𝙡𝙖𝙢𝙖𝙩 𝙙𝙖𝙩𝙖𝙣𝙜 𝙙𝙞 ${metadata.subject}, 𝙟𝙖𝙣𝙜𝙖𝙣 𝙡𝙪𝙥𝙖 𝙥𝙖𝙩𝙪𝙝𝙞 𝙖𝙩𝙪𝙧𝙖𝙣 𝙜𝙧𝙤𝙪𝙥 𝙮𝙖... @${num.split("@")[0]}` })
                } else if (anu.action == 'remove') {
                    hisoka.sendMessage(anu.id, { image: { url: ppuser }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} 𝙔𝙖𝙝𝙝𝙝, 𝙗𝙚𝙗𝙖𝙣 𝙂𝘾 𝙤𝙪𝙩 𝙣𝙞𝙝😣.. 𝙆𝙚𝙣𝙖 𝙢𝙚𝙣𝙩𝙖𝙡 𝙠𝙖𝙝  ${metadata.subject}` })
                }
            }
        } catch (err) {
            console.log(err)
